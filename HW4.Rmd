...
matrix5 = data.frame()
matrix5 = cbind(sort(psych$age),rep(0,nrow(psych)))
colnames(matrix5) = c('age','death')
matrix6 = cbind(psych$agec,psych$death)
matrix5 = rbind(matrix5,matrix6)
matrix5 = as.data.frame(matrix5[order(matrix5[,1]),])
matrix3 = cbind(sort(unique(matrix5$age)),tapply(matrix5$death, matrix5$age, sum),table(matrix5$age))
matrix3 = as.data.frame(matrix3[order(matrix3[,1]),])

rownames(matrix3) = c(1:nrow(matrix3))
colnames(matrix3) = c('age','d','sum')

for (i in 1:nrow(matrix3)) {
  if(i == 1){
    matrix3$y[i] = matrix3$sum[i]
    }
  else{
    matrix3$y[i] = matrix3$y[i-1] +matrix3$sum[i]-matrix3$d[i-1]
  }
  }
