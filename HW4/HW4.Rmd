...
plot(kmfitCAN, cumhaz=TRUE)

```

It seems propotional.
...
source(paste0('~/Biostats-M215/', "bc_median_ci.R"))
...
# (bc.median.ci(kmfitCAN, table = TRUE, 'asin'))
```
median for cllride groip not avalaible. 

## (d)
```{r}
source(paste0('~/Biostats-M215/', "log_ci.R"))
source(paste0('~/Biostats-M215/', "arcsin_ci.R"))
source(paste0('~/Biostats-M215/', "LRci.R"))
source(paste0('~/Biostats-M215/', "SurvivalProb_boots.R"))
...
(LRci.surv(kmfitPIAN, t = 10))
(LRci.surv(kmfitCAN, t = 10))
```
...
## (e)
```{r}
source(paste0('~/Biostats-M215/', "conf_bands.R"))
...
cbands.interval(kmfitCAN, tL = 8, tU = 20, crit.value = 2.5245,type = 'asin', method = 'ep')
```
...
for (i in 1:nrow(matrix3)) {
  if(i == 1){
    ...
    matrix3$y[i] = matrix3$y[i-1] +matrix3$incI[i]-matrix3$d[i-1]-matrix3$censorI[i]
  }
  }
